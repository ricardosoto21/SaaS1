...
export async function POST(request: Request) {
  ...
  if (contentLength > 65_536) return NextResponse.json({ error: "payload_too_large" }, { status: 413 });
  ...
  if (existing?.processing_status === "processed") { logEvent("info", "sumup_webhook_duplicate"); return new NextResponse(null, { status: 204 }); }
  ...
  try {
    ...
    logEvent("info", "sumup_webhook_processed");
    ...
    logEvent("error", "sumup_webhook_verification_failed");
