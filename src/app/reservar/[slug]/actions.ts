...
export async function createPublicBookingHoldAction(formData: FormData) {
  ...
  redirect(`/reservar/${slug}?hold=${encodeURIComponent(String(data.holdId))}&deposit=${encodeURIComponent(String(data.depositAmount))}&token=${encodeURIComponent(accessToken)}`);
}
...
export async function startPublicBookingCheckoutAction(formData: FormData) {
  ...
  if (!slug || !UUID.test(holdId) || token.length < 24) redirect(`/reservar/${slug}?error=Reserva%20invalida`);
  ...
  try {
    const url = await createBookingCheckout(holdId, `${origin}/reservar/${encodeURIComponent(slug)}/gestionar?hold=${encodeURIComponent(holdId)}&token=${encodeURIComponent(token)}&payment=returned`);
    redirect(url);
  } catch {
    redirect(`/reservar/${slug}?hold=${encodeURIComponent(holdId)}&token=${encodeURIComponent(token)}&error=Pago%20no%20disponible`);
  }
}
export async function managePublicBookingAction(formData: FormData) {
  ...
  if (!(await enforcePublicRateLimit("booking-management", `${slug}:${holdId}`, 10, 900))) redirect(`/reservar/${slug}/gestionar?hold=${encodeURIComponent(holdId)}&token=${encodeURIComponent(token)}&error=Intenta%20nuevamente%20más%20tarde`);
  const supabase = await getSupabaseServerClient();
  ...
  const base = `/reservar/${slug}/gestionar?hold=${encodeURIComponent(holdId)}&token=${encodeURIComponent(token)}`;
